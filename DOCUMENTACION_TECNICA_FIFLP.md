...
- Capa de bloques internos: texto, imagen, texto-imagen, galería masonry, hero, prólogos, etc.
...
- imagen
- texto-imagen
- galería masonry (columnas CSS, lightbox)
- responsive
- capítulo
- imagen full bleed
- layout editorial
...
Los grupos exportables viven en `acf-json/` del tema hijo. Los más relevantes para la composición son:

- `group_bloques_editoriales.json` — flexible `bloques` en páginas editoriales.
- `group_secciones_onepage.json` — CPT secciones onepage (`modulos_onepage`).
- `group_cronologias_editoriales.json` — hitos del CPT cronología.

Tras cambios en JSON, sincronizar en WordPress (ACF) si la sincronización de grupos está activa.

## 10.15 Galería masonry (bloque, onepage, cronología)

**Plantilla:** `template-parts/bloques/galeria-masonry.php`

**Uso:**

- **Página editorial:** layout flexible `galeria_masonry` en `bloques` (campos: `galeria_imagenes`, `columnas_masonry` 2/3/4, `titulo` opcional, `sin_redondeo`). Render vía `fiflp_render_editorial_block_layout()`.
- **Onepage:** mismo `name` `galeria_masonry` dentro de `modulos_onepage`; la sección carga el partial con `$args['module']` y `onepage => true`. Opcionales `mostrar_en_submenu` y `titulo_submenu` para el índice lateral.
- **Cronología:** en cada hito, campos opcionales `galeria_masonry` (galería) y `galeria_masonry_columnas`. Se renderiza dentro del hito en `cronologia-editorial.php` (tras el texto, antes de las imágenes del hito). No sustituye ni renombra campos de nodos existentes.

**Front:** layout tipo masonry con `column-count` en `.galeria-masonry__grid`; miniaturas con `.lightbox-trigger` y `data-caption` (mismo patrón que el resto del editorial).

**Admin cronología:** `assets/js/acf-cronologia-editorial-admin.js` y `assets/css/acf-cronologia-editorial-admin.css` envuelven la galería del hito en `.fiflp-crono-hito-grupo-galeria` (ancho completo bajo los grupos Texto / Imagen).

**Texto + imagen (escala visual):** el bloque añade clases `texto-imagen--grid-*` según la escala ACF para repartir columnas del grid; imágenes &lt;100% usan tope `cqw` respecto al ancho del bloque (detalle en `style.css`).

**Onepage — fondo:** `template-parts/bloques/seccion-onepage.php` acepta en la paleta cerrada también `#ffffff` y `#1e1e1e`, y aplica `seccion-onepage__shell--dark-bg` tanto para `#072728` como para `#1e1e1e`.
